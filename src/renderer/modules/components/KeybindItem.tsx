...
type RecorderType = React.FC<React.PropsWithRef<RecorderProps>>;
...
const getKeybindItem = async (): Promise<{
  KeybindItem: KeybindItemType;
  Keybind: KeybindType;
}> => {
  const Recorder = await waitForModule<RecorderType>(filters.bySource("handleComboChange"));

  const Keybind = (props: KeybindProps): React.ReactElement => {
  ...
  const KeybindItem = (props: React.PropsWithChildren<KeybindItemProps>): React.ReactElement => {
    return (
      <FormItem
  ...
  return { Keybind, KeybindItem };
};

export default getKeybindItem();
