...
async function getTheme(path: string): Promise<RepluggedTheme> {
  ...
  if (isAsar)
    await extractAddon(themePath, realThemePath);
...
ipcMain.handle(
  RepluggedIpcChannels.LIST_THEMES,
  async (): Promise<RepluggedTheme[]> => {
    const themes = [];
...
ipcMain.handle(RepluggedIpcChannels.UNINSTALL_THEME, (_, themeName: string) => {
  ...
  const realThemePath = isAsar ? join(TEMP_THEMES_DIR, themeName.replace(/\.asar$/, "")) : themePath; // Remove ".asar" from the directory name
...
ipcMain.on(RepluggedIpcChannels.CLEAR_TEMP_THEME, () => { try { rmdirSync(TEMP_THEMES_DIR, { recursive: true }) } catch { } });
